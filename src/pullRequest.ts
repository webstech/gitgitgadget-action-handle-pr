...
import { handlePRUpdate, PRUpdateInterface } from "gitgitgadget/lib/pull-action";
...
async function run(): Promise<void> {
    try {
        ...
        */
        ...
        const prParms: PRUpdateInterface = { ...parms };
        await handlePRUpdate(prParms);
...
function getParms(): parmInterface {
    ...
    if (!parms.action || !["comment", "push"].includes(parms.action)) {
        throw new Error(`Missing or invalid required input 'action'.`);
    }
